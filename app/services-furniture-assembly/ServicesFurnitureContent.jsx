...
export default function ServicesFurniturePostContent() {
 ...
 const [amount, setAmount] = useState(""); 
  ...
  useEffect(() => {
    const fetchDraftData = async () => {
      ...
      try  {
        ...
        setServiceTitle(serviceAd.serviceTitle || "");
        setServiceDuration(serviceAd.serviceDuration || "");
        setServiceExperience(serviceAd.serviceExperience || "");
        setServiceAvailability(serviceAd.serviceAvailability || "");
        const serveLocation = Array.isArray(servicesAd.serviceLocation)
          ? serviceAd.serviceLocation 
          : [];
        setServiceLocation(serveLocation);
        setYearOfExperience(serviceAd.yearOfExperience || "");
        setPricingType(serviceAd.pricingType || "");
        setAmount(serviceAd.amount?.toString() || "");
        setServiceDiscount(serviceAd.serviceDiscount || "");
        setNegotiation(serviceAd.negotiation || "");
        setDescription(serviceAd.description || "");

        // ✅ Set business from either petsAd or carAd
        const businessId = serviceAd.businessCategory?._id 
          || serviceAd.businessCategory 
        ...
        setEditingCarAd({
          ...
          category: carAd?.category || 'Lessons & Training',
          location: carAd?.location || '',
          images: carAd?.serviceImage || [],
        });
  ...
  const submitAd = useCallback(async (planToSubmit, useWallet = false) => {
    try {
      ...
      const res = await api.post("/services/create-services-ad", payload);
    ...
    serviceTitle, serviceDuration, serviceExperience, serviceAvailability, serviceLocation, yearOfExperience, pricingType,  amount, serviceDiscount, negotiation, business, description,
  ...
  const handleSaveAsDraft = useCallback(async () => {
    try {
      ...
      const res = await api.post("/services/save-draft", payload);
    ...
    <>
      <div className="bg-white shadow-phenom rounded-[12px] p-5 md:p-10">
        ...
        <h3 className="text-[#525252] font-[500] font-inter text-[16px] mb-4 text-left md:text-center">
          {editingCarAd ? "Complete Ads for Services" : "Post a Service Ad"}
        </h3>
        ...
        <form>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
            ...
            value={serviceDuration}
              ...
              placeholder="2hrs or 2 hours per session"
            />
            <PostDropdown
              label="Experience Level"
              value={serviceExperience}
              onChange={setServiceExperience}
              options={[
                "Beginner",
                "Intermediate",
                "Expert",
            ...
            <PostDropdown 
              ...
              onChange={setServiceAvailability}
              options={[
                "Full-Time",
                "Part-Time",
                "On Appointment",
          ...
          <div className="mt-4">
            ...
            <textarea
              placeholder="Describe your service, include what you offer, your experience, tools or skills used, and what makes your service unique"
              value={description}
