...
export default function ConstructionTilesPostContent() {
  ...
  useEffect(() => {
    ...
    if (!isEditMode && isDraftMode && !idToUse) {
     ...
     let carAd = null;
       ...
       setSize(constructionAd.size || "");
       setConstructionUnit(constructionAd.constructionUnit || "");
       setConstructionBrand(constructionAd.constructionBrand || "");
         if (constructionAd.bulkPrice && constructionAd.bulkPrice.length > 0) {
               setBulkPrices(constructionAd.bulkPrice.map(bp => ({
     ...
     const businessId = constructionAd.businessCategory?._id 
       ...
       setEditingCarAd({
         ...
         category: carAd?.category || 'Flooring & Tiles',
  ...
  const submitAd = useCallback(async (planToSubmit, useWallet = false) => {
   ...
   constructionTitle, constructionType, condition, constructionUnit, constructionBrand, bulkPrices, size,  amount, negotiation, business, description,
    ...
    <>
      <div className="bg-white shadow-phenom rounded-[12px] p-5 md:p-10">
        ...
        <h3 className="text-[#525252] font-[500] font-inter text-[16px] mb-4 text-left md:text-center">
          {editingCarAd ? "Complete Your Flooring & Tiles Ad" : "Post Flooring & TIles Ad"}
        </h3>
