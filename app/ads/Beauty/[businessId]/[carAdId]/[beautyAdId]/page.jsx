...
export default function BeautyDetails() {
  ...
  const [offerAmount, setOfferAmount] = useState("");
    const { businessId, carAdId, beautyAdId } = useParams();
    const [carAd, setCarAd] = useState(null);
    const [showDetails, setShowDetails] = useState(false);
    const [beautyAd, setBeautyAd] = useState(null);
    ...
    useEffect(() => {
      const fetchAds = async  () => {
        ...
        try {
         ...
         if (beautyAdId && beautyAdId !== "null") {
          const beautyRes = await api.get(`/beauty/get-beauty/${beautyAdId}`);
          setBeautyAd(beautyRes.data.data || null);
        }
        } catch (error) {
          setCarAd(null);
          setBeautyAd(null);
   ...
   if (!carAd && !beautyAd) return <div>Ad not found.</div>;
    ...
    return  (
      ...
      <div className="md:w-1/3 w-full grid grid-cols-2 grid-rows-2 gap-2">
       {carAd?.beautyImage &&
         carAd.beautyImage.slice(1, 5).map((img, idx) => (
  ...
  <div className="mt-4">
   ...
   {/* Right Section */}
     ...
     <div className="flex justify-between items-center p-4">
       ...
       {beautyAd && (
        <span className="text-[#525252] md:text-[24px] font-[500] font-inter">₦{beautyAd.amount?.toLocaleString()}</span>
       )}
     </div>
     <div className="p-4">
      {showInput ? (
