...
export default function LaptopDetails() {
  ...
  const [offerAmount, setOfferAmount] = useState("");
    const { businessId, carAdId, laptopAdId } = useParams();
    const [carAd, setCarAd] = useState(null);
    const [showDetails, setShowDetails] = useState(false);
    const [laptopAd, setLaptopAd] = useState(null);
    ...
    useEffect(() => {
      const fetchAds = async  () => {
        ...
        try {
         ...
         if (laptopAdId && laptopAdId !== "null") {
          const laptopRes = await api.get(`/laptops/get-laptop/${laptopAdId}`);
          setLaptopAd(laptopRes.data.data || null);
        }
        } catch (error) {
          setCarAd(null);
          setLaptopAd(null);
   ...
   if (!carAd && !laptopAd) return <div>Ad not found.</div>;
    ...
    return  (
      ...
      <div className="md:w-1/3 w-full grid grid-cols-2 grid-rows-2 gap-2">
       {carAd?.laptopImage &&
         carAd.laptopImage.slice(1, 5).map((img, idx) => (
  ...
  <div className="mt-4">
    <div className="flex flex-wrap justify-between gap-y-4 gap-x-[4%] max-w-[650px] mx-auto">
      ...
      {/* Only show SIM Type if it exists and is not empty */}
      ...
      {laptopAd?.laptopScreenSize && laptopAd.laptopScreenSize.trim() !== "" && (
        <div className="flex flex-col w-[48%] md:w-[30%]">
          <span className="text-[#868686] text-[12px] md:text-[14px] font-medium font-inter">
            Screen Size 
          </span>
  ...
  <div className="mt-4">
   ...
   {/* Right Section */}
     ...
     <div className="flex justify-between items-center p-4">
       ...
       {laptopAd && (
        <span className="text-[#525252] md:text-[24px] font-[500] font-inter">₦{laptopAd.amount?.toLocaleString()}</span>
       )}
     </div>
     <div className="p-4">
      {showInput ? (
