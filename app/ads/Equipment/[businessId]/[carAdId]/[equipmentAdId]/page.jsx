...
export default function EquipmentDetails() {
  ...
  const [offerAmount, setOfferAmount] = useState("");
    const { businessId, carAdId, equipmentAdId } = useParams();
    const [carAd, setCarAd] = useState(null);
    const [showDetails, setShowDetails] = useState(false);
    const [equipmentAd, setEquipmentAd] = useState(null);
    ...
    useEffect(() => {
      const fetchAds = async  () => {
        ...
        try {
         ...
         if (equipmentAdId && equipmentAdId !== "null") {
          const equipmentRes = await api.get(`/equipments/get-equipment/${equipmentAdId}`);
          setEquipmentAd(equipmentRes.data.data || null);
        }
        } catch (error) {
          setCarAd(null);
          setEquipmentAd(null);
      ...
      fetchAds();
      fetchProfile();
    }, [carAdId, equipmentAdId, businessId]);
   ...
   if (!carAd && !equipmentAd) return <div>Ad not found.</div>;
    ...
    return  (
      ...
      <div className="md:w-1/3 w-full grid grid-cols-2 grid-rows-2 gap-2">
       {carAd?.equipmentImage &&
         carAd.equipmentImage.slice(1, 5).map((img, idx) => (
  ...
  <div className="mt-4">
    <div className="flex flex-wrap justify-between gap-y-4 gap-x-[4%] max-w-[650px] mx-auto">
     ...
     {equipmentAd?.usageType && (
        <div className="flex flex-col w-[48%] md:w-[30%]">
          <span className="text-[#868686] text-[12px] md:text-[14px] font-medium font-inter">Usage  Type</span>
          <div className="mt-2">
            {equipmentAd && (
              <span className="text-[#868686] text-[12px] md:text-[14px] font-[500] font-inter">
              {equipmentAd.usageType}
  ...
  <div className="mt-4">
   ...
   {/* Right Section */}
     ...
     <div className="flex justify-between items-center p-4">
       ...
       {equipmentAd && (
        <span className="text-[#525252] md:text-[24px] font-[500] font-inter">₦{equipmentAd.amount?.toLocaleString()}</span>
       )}
     </div>
     <div className="p-4">
      {showInput ? (
