...
export default function HouseholdDetails() {
    ...
    const { businessId, carAdId,  householdAdId } = useParams();
    const [carAd, setCarAd] = useState(null);
    const [showDetails, setShowDetails] = useState(false);
    const [householdAd, setHouseholdAd] = useState(null);
    ...
    useEffect(() => {
      const fetchAds = async  () => {
        ...
        try {
         ...
         if (householdAdId && householdAdId !== "null") {
          const householdRes = await api.get(`/household/get-household/${householdAdId}`);
          setHouseholdAd(householdRes.data.data || null);
        }
        } catch (error) {
          setCarAd(null);
          setHouseholdAd(null);
      ...
      fetchAds();
      fetchProfile();
    }, [carAdId, householdAdId, businessId]);
   ...
   if (!carAd && !householdAd) return <div>Ad not found.</div>;
    ...
    return  (
      ...
      <div className="md:w-1/3 w-full grid grid-cols-2 grid-rows-2 gap-2">
       {carAd?.householdImage &&
         carAd.householdImage.slice(1, 5).map((img, idx) => (
  ...
  <div className="mt-4">
   ...
   {/* Right Section */}
     ...
     <div className="flex justify-between items-center p-4">
       ...
       {householdAd && (
        <span className="text-[#525252] md:text-[24px] font-[500] font-inter">₦{householdAd.amount?.toLocaleString()}</span>
       )}
     </div>
     <div className="p-4">
      {showInput ? (
