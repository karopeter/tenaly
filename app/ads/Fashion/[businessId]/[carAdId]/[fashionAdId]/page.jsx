...
export default function FashionDetails() {
  ...
  const [offerAmount, setOfferAmount] = useState("");
    const { businessId, carAdId, fashionAdId } = useParams();
    const [carAd, setCarAd] = useState(null);
    const [showDetails, setShowDetails] = useState(false);
    const [fashionAd, setFashionAd] = useState(null);
    ...
    useEffect(() => {
      const fetchAds = async  () => {
        ...
        try {
         ...
         if (fashionAdId && fashionAdId !== "null") {
          const fashionRes = await api.get(`/fashion/get-fashion/${fashionAdId}`);
          setFashionAd(fashionRes.data.data || null);
        }
        } catch (error) {
          setCarAd(null);
          setFashionAd(null);
      ...
      fetchAds();
      fetchProfile();
    }, [carAdId, fashionAdId, businessId]);
   ...
   if (!carAd && !fashionAd) return <div>Ad not found.</div>;
    ...
    return  (
      ...
      <div className="md:w-1/3 w-full grid grid-cols-2 grid-rows-2 gap-2">
       {carAd?.fashionImage &&
         carAd.fashionImage.slice(1, 5).map((img, idx) => (
  ...
  <div className="mt-4">
   ...
   {/* Right Section */}
     ...
     <div className="flex justify-between items-center p-4">
       ...
       {fashionAd && (
        <span className="text-[#525252] md:text-[24px] font-[500] font-inter">₦{fashionAd.amount?.toLocaleString()}</span>
       )}
     </div>
     <div className="p-4">
      {showInput ? (
