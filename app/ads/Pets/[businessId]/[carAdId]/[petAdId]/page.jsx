...
export default function PetDetails() {
  ...
  const [offerAmount, setOfferAmount] = useState("");
    const { businessId, carAdId, petAdId  } = useParams();
    const [carAd, setCarAd] = useState(null);
    const [showDetails, setShowDetails] = useState(false);
    const [petsAd, setPetsAd] = useState(null);
    ...
    useEffect(() => {
      const fetchAds = async  () => {
        ...
        try {
         ...
         if (petAdId && petAdId !== "null") {
          const petRes = await api.get(`/pets/get-pet/${petAdId}`);
          setPetsAd(petRes.data.data || null);
        }
        } catch (error) {
          setCarAd(null);
          setPetsAd(null);
   ...
   if (!carAd && !petAdId) return <div>Ad not found.</div>;
    ...
    return  (
      ...
      <div className="md:w-1/3 w-full grid grid-cols-2 grid-rows-2 gap-2">
       {carAd?.petsImage &&
         carAd.petsImage.slice(1, 5).map((img, idx) => (
   ...
   <div className="bg-[#FAFAFA] w-full rounded-[8px]">
     <div className="flex justify-between items-center p-4">
       ...
       {petsAd && (
        <span className="text-[#525252] text-[24px] font-[500] font-inter">₦{petsAd.amount?.toLocaleString()}</span>
       )}
     </div>
     <div className="p-4">
      {showInput ? (
             ...
             <div className="mt-4">
                {petsAd && (
                 <span className="text-[#868686] text-[10px] md:text-[12px] font-[400] font-inter">
                  Posted on {new Date(petsAd.createdAt).toLocaleDateString("en-US", {
                  year: "numeric",
                  month: "long",
                  day: "numeric"
                })}
  ...
  <div className="mt-4">
    ...
    {petsAd?.age && (
       <div className="flex flex-col w-[48%] md:w-[30%]">
        <span className="text-[#868686] text-[12px] md:text-[14px] font-medium font-inter">Age</span>
        <span className="text-[#525252] mt-2 text-[14px] md:text-[16px] font-medium font-inter">
          {petsAd.age}
        </span>
      </div>
  ...
  <div className="mt-4">
   ...
   {/* Right Section */}
     ...
     <div className="flex justify-between items-center p-4">
       ...
       {petsAd && (
        <span className="text-[#525252] md:text-[24px] font-[500] font-inter">₦{petsAd.amount?.toLocaleString()}</span>
       )}
     </div>
     <div className="p-4">
      {showInput ? (
