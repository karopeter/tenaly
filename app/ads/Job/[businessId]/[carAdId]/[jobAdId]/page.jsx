...
export default function JobDetails() {
  ...
  const [offerAmount, setOfferAmount] = useState("");
    const { businessId, carAdId, jobAdId } = useParams();
    ...
    useEffect(() => {
      const fetchAds = async  () => {
        ...
        try {
         ...
         if (jobAdId && jobAdId !== "null") {
          const jobRes = await api.get(`/jobs/get-job/${jobAdId}`);
           setJobAd(jobRes.data.data || null);
        }
        } catch (error) {
          setCarAd(null);
         setJobAd(null);
   ...
   if (!carAd && !jobAd) return <div>Ad not found.</div>;
    ...
    return  (
      ...
      <div className="md:w-1/3 w-full grid grid-cols-2 grid-rows-2 gap-2">
       {carAd?.jobImage &&
         carAd.jobImage.slice(1, 5).map((img, idx) => (
  ...
  <div className="mt-4">
   ...
   {/* Right Section */}
     ...
     <div className="flex justify-between items-center p-4">
       ...
       {jobAd  && (
        <span className="text-[#525252] md:text-[24px] font-[500] font-inter">₦{jobAd.salaryRange?.toLocaleString()}</span>
       )}
     </div>
     <div className="p-4">
      {showInput ? (
