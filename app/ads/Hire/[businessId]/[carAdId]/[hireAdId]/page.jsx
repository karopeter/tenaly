...
export default function HireDetails() {
  ...
  const [offerAmount, setOfferAmount] = useState("");
    const { businessId, carAdId, hireAdId } = useParams();
    const [carAd, setCarAd] = useState(null);
    const [showDetails, setShowDetails] = useState(false);
    const [hireAd, setHireAd] = useState(null);
    ...
    useEffect(() => {
      const fetchAds = async  () => {
        ...
        try {
         ...
         if (hireAdId && hireAdId !== "null") {
          const hireRes = await api.get(`/hire/get-hire/${hireAdId}`);
           setHireAd(hireRes.data.data || null);
        }
        } catch (error) {
          setCarAd(null);
         setHireAd(null);
   ...
   if (!carAd && !hireAd) return <div>Ad not found.</div>;
    ...
    return  (
      ...
      <div className="md:w-1/3 w-full grid grid-cols-2 grid-rows-2 gap-2">
       {carAd?.hireImage &&
         carAd.hireImage.slice(1, 5).map((img, idx) => (
   ...
   <div className="bg-[#FAFAFA] w-full rounded-[8px]">
     <div className="flex justify-between items-center p-4">
       <span className="text-[#525252] text-[15px] font-[400] font-inter">Salary Range</span>
       {hireAd && (
  ...
  <div className="mt-4">
   ...
   {/* Right Section */}
     ...
     <div className="flex justify-between items-center p-4">
       <span className="text-[#525252] md:text-[15px] font-[400] font-inter">Salary Range</span>
       {hireAd && (
        <span className="text-[#525252] md:text-[24px] font-[500] font-inter">₦{hireAd.salaryRange?.toLocaleString()}</span>
       )}
     </div>
     <div className="p-4">
      {showInput ? (
