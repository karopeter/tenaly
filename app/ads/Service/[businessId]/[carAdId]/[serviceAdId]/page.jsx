...
export default function ServiceDetails() {
  ...
  const [offerAmount, setOfferAmount] = useState("");
    const { businessId, carAdId, serviceAdId } = useParams();
    const [carAd, setCarAd] = useState(null);
    const [showDetails, setShowDetails] = useState(false);
    const [serviceAd, setServiceAd] = useState(null);
    ...
    useEffect(() => {
      const fetchAds = async  () => {
        ...
        try {
         ...
         if (serviceAdId && serviceAdId !== "null") {
          const serviceRes = await api.get(`/services/get-services/${serviceAdId}`);
          setServiceAd(serviceRes.data.data || null);
        }
        } catch (error) {
          setCarAd(null);
          setServiceAd(null);
      ...
      fetchAds();
      fetchProfile();
    }, [carAdId, serviceAdId, businessId]);
   ...
   if (!carAd && !serviceAd) return <div>Ad not found.</div>;
    ...
    return  (
      ...
      <div className="md:w-1/3 w-full grid grid-cols-2 grid-rows-2 gap-2">
       {carAd?.serviceImage &&
         carAd.serviceImage.slice(1, 5).map((img, idx) => (
           ...
           <div className="bg-[#FAFAFA] w-full md:w-[650px] h-auto  rounded-[12px] p-4 md:p-6 mt-5">
             ...
             <div className="flex flex-row gap-4 mt-2 flex-wrap">
               ...
               <div className="flex items-center gap-2">
                 ...
                 {serviceAd && (
                 <span className="text-[#868686] text-[12px] md:text-[14px] font-[500] font-inter">
                    Service Availability: {serviceAd.serviceAvailability}
                  </span>
                 )}
               </div>
             </div>
  ...
  <div className="mt-4">
    <div className="flex flex-wrap justify-between gap-y-4 gap-x-[4%] max-w-[650px] mx-auto">
     ...
     {serviceAd?.pricingType && (
        <div className="flex flex-col w-[48%] md:w-[30%]">
          <span className="text-[#868686] text-[12px] md:text-[14px] font-medium font-inter">Pricing Type</span>
          <div className="mt-2">
            {serviceAd && (
              <span className="text-[#868686] text-[12px] md:text-[14px] font-[500] font-inter">
              {serviceAd.pricingType}
      ...
      {serviceAd?.serviceDuration && (
        <div className="flex flex-col w-[48%] md:w-[30%]">
          <span className="text-[#868686] text-[12px] md:text-[14px] font-medium font-inter">Service Duration</span>
          <span className="text-[#525252] mt-2 text-[14px] md:text-[16px] font-medium font-inter">
            {serviceAd.serviceDuration}
          </span>
        </div>
        ...
        <div className="flex flex-col w-[48%] md:w-[30%]">
          <span className="text-[#868686] text-[12px] md:text-[14px] font-medium font-inter">Service Location</span>
          <span className="text-[#525252] mt-2 text-[14px] md:text-[16px] font-medium font-inter">
             {serviceAd.serviceLocation}
          </span>
        </div>
       ...
       {serviceAd?.serviceDiscount && (
        <div className="flex flex-col w-[48%] md:w-[30%]">
          <span className="text-[#868686] text-[12px] md:text-[14px] font-medium font-inter">Service Discount</span>
          <span className="text-[#525252] mt-2 text-[14px] md:text-[16px] font-medium font-inter">
             {serviceAd.serviceDiscount}
          </span>
        </div>
  ...
  <div className="mt-4">
   ...
   {/* Right Section */}
     ...
     <div className="flex justify-between items-center p-4">
       ...
       {serviceAd && (
        <span className="text-[#525252] md:text-[24px] font-[500] font-inter">₦{serviceAd.amount?.toLocaleString()}</span>
       )}
     </div>
     <div className="p-4">
      {showInput ? (
