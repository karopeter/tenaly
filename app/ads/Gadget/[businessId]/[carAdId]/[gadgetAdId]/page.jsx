...
export default function GadgetDetails() {
  ...
  const [offerAmount, setOfferAmount] = useState("");
    const { businessId, carAdId, gadgetAdId } = useParams();
    const [carAd, setCarAd] = useState(null);
    const [showDetails, setShowDetails] = useState(false);
    const [gadgetAd, setGadgetAd] = useState(null);
    ...
    useEffect(() => {
      const fetchAds = async  () => {
        ...
        try {
         ...
         if (gadgetAdId && gadgetAdId !== "null") {
          const gadgetRes = await api.get(`/gadget/get-gadget/${gadgetAdId}`);
          setGadgetAd(gadgetRes.data.data || null);
        }
        } catch (error) {
          setCarAd(null);
          setGadgetAd(null);
   ...
   if (!carAd && !gadgetAd) return <div>Ad not found.</div>;
    ...
    return  (
      ...
      <div className="md:w-1/3 w-full grid grid-cols-2 grid-rows-2 gap-2">
       {carAd?.gadgetImage &&
         carAd.gadgetImage.slice(1, 5).map((img, idx) => (
  ...
  <div className="mt-4">
   ...
   {/* Right Section */}
     ...
     <div className="flex justify-between items-center p-4">
       ...
       {gadgetAd && (
        <span className="text-[#525252] md:text-[24px] font-[500] font-inter">₦{gadgetAd.amount?.toLocaleString()}</span>
       )}
     </div>
     <div className="p-4">
      {showInput ? (
