...
export default function AgricultureDetails() {
  ...
  const [offerAmount, setOfferAmount] = useState("");
    const { businessId, carAdId, kidAdId } = useParams();
    const [carAd, setCarAd] = useState(null);
    const [showDetails, setShowDetails] = useState(false);
    const [kidAd, setKidAd] = useState(null);
    ...
    useEffect(() => {
      const fetchAds = async  () => {
        ...
        try {
         ...
         if (kidAdId && kidAdId !== "null") {
          const kidRes = await api.get(`/kids/get-kid/${kidAdId}`);
          setKidAd(kidRes.data.data || null);
        }
        } catch (error) {
          setCarAd(null);
          setKidAd(null);
   ...
   if (!carAd && !kidAd) return <div>Ad not found.</div>;
    ...
    return  (
      ...
      <div className="md:w-1/3 w-full grid grid-cols-2 grid-rows-2 gap-2">
       {carAd?.kidsImage &&
         carAd.kidsImage.slice(1, 5).map((img, idx) => (
   ...
   <div className="bg-[#FAFAFA] w-full rounded-[8px]">
     <div className="flex justify-between items-center p-4">
       ...
       {kidAd && (
        <span className="text-[#525252] text-[24px] font-[500] font-inter">₦{kidAd.amount?.toLocaleString()}</span>
       )}
     </div>
     <div className="p-4">
      {showInput ? (
             ...
             <div className="mt-4">
                {kidAd && (
                 <span className="text-[#868686] text-[10px] md:text-[12px] font-[400] font-inter">
                  Posted on {new Date(kidAd.createdAt).toLocaleDateString("en-US", {
                  year: "numeric",
                  month: "long",
                  day: "numeric"
                })}
  ...
  <div className="mt-4">
    <div className="flex flex-wrap justify-between gap-y-4 gap-x-[4%] max-w-[650px] mx-auto">
     ...
     {kidAd?.ageGroup && (
        <div className="flex flex-col w-[48%] md:w-[30%]">
          <span className="text-[#868686] text-[12px] md:text-[14px] font-medium font-inter">Age Group</span>
          <div className="mt-2">
            {kidAd && (
              <span className="text-[#868686] text-[12px] md:text-[14px] font-[500] font-inter">
              {kidAd.ageGroup}
  ...
  <div className="mt-4">
   ...
   {/* Right Section */}
     ...
     <div className="flex justify-between items-center p-4">
       ...
       {kidAd && (
        <span className="text-[#525252] md:text-[24px] font-[500] font-inter">₦{kidAd.amount?.toLocaleString()}</span>
       )}
     </div>
     <div className="p-4">
      {showInput ? (
