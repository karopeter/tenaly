...
export default function AgroChemicalContent() {
        ...
        setFormulationType(formulations);
         ...
         const businessId = agriAd.businessCategory?._id 
            ...
            setEditingCarAd({
              ...
              category: carAd?.category || 'Agro Chemicals (pesticides, herbicides)',
  ...
  const buildPayload = (planType, useWallet = false) => {
    const payload = {
      ...
      negotiation,
      formulationType: Array.isArray(formulationType)
         ? formulationType.map((f) => (typeof f == "string" ? f : f.name))
         : [],
  ...
  const submitAd = useCallback(async (planToSubmit, useWallet = false) => {
    ...
    title, feedType, brand, condition, formulationType, amount, negotiation, business, description,
      ...
      <div className="bg-white shadow-phenom rounded-[12px] p-5 md:p-10">
        ...
        <form>
          ...
          <div className="mt-4">
            ...
            <textarea
              placeholder="Enter the description of the Agro Chemical"
              value={description}
