...
export default function GadgetScreenProtectorPostContent() {
  ...
  useEffect(() => {
    ...
    if (!isEditMode && isDraftMode && !idToUse) {
      ...
      try {
       const gadgetResponse = await api.get(`/gadget/draft/${idToUse}`);
                 
       if (!gadgetResponse.data || !gadgetResponse.data.gadgetAd) {
         console.log("⚠️ No Gadget draft found");
         setIsLoadingDraft(false);
         return;
      }   
         
      const gadgetAd = gadgetResponse.data.gadgetAd;
      console.log("✅ Loaded GadgetAd draft:", gadgetAd);
     ...
     let carAd = null;
       ...
       setRam(gadgetAd.ram || "");
       setOperatingSystem(gadgetAd.operatingSystem || "");
       setSimType(gadgetAd.simType || "");
     ...
     const businessId = gadgetAd.businessCategory?._id 
       ...
       setEditingCarAd({
         ...
         category: carAd?.category || 'Screen Protectors',
  ...
  const submitAd = useCallback(async (planToSubmit, useWallet = false) => {
    try {
      ...
      const res = await api.post("/gadget/create-gadget-ad", payload);
  ...
  const handleSaveAsDraft = useCallback(async () => {
    try {
      ...
      const res = await api.post("/gadget/save-draft", payload);

      const savedPlan = res.data.data?.plan || 'free';

      toast.success(`Gadget ad saved as draft with ${savedPlan} plan!`);
    ...
    <>
      <div className="bg-white shadow-phenom rounded-[12px] p-5 md:p-10">
        ...
        <h3 className="text-[#525252] font-[500] font-inter text-[16px] mb-4 text-left md:text-center">
          {editingCarAd ? "Complete Your Gadget Ad" : "Post Gadget Ad"}
        </h3>
        ...
        <form>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
            ...
            <InputField
               ...
               placeholder="e.g charger, box, case, earpiece"
               value={accessories}
