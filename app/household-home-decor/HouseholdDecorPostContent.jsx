 ...
 const [householdStyle, setHouseholdStyle] = useState("");
  ...
  useEffect(() => {
    ...
    if (!isEditMode && isDraftMode && !idToUse) {
      ...
      try {
       const householdResponse = await api.get(`/household/draft/${idToUse}`);
                 
       if (!householdResponse.data || !householdResponse.data.householdAd) {
         console.log("⚠️ No Household draft found");
         setIsLoadingDraft(false);
         return;
      }   
         
      const householdAd = householdResponse.data.householdAd;
      console.log("✅ Loaded HouseholdAd draft:", householdAd);
     ...
     let carAd = null;
       ...
       setHouseholdStyle(householdAd.householdStyle || "");
     ...
     setDescription(householdAd.description || "");
         
     const businessId = householdAd.businessCategory?._id 
           || householdAd.businessCategory 
       ...
       setEditingCarAd({
         ...
         category: carAd?.category || 'Home Decor',
  ...
  const submitAd = useCallback(async (planToSubmit, useWallet = false) => {
    try {
      ...
      const res = await api.post("/household/create-household-ad", payload);
    ...
    householdTitle, householdType, condition, householdBrand, householdMaterial, householdStyle, householdColor, amount, negotiation, business, description,
  ...
  const handleSaveAsDraft = useCallback(async () => {
    try {
      ...
      const res = await api.post("/household/save-draft", payload);
    ...
    <>
      <div className="bg-white shadow-phenom rounded-[12px] p-5 md:p-10">
        ...
        <h3 className="text-[#525252] font-[500] font-inter text-[16px] mb-4 text-left md:text-center">
          {editingCarAd ? "Complete Your Home Decor Ad" : "Post Home Decor Ad"}
        </h3>
        ...
        <form>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
            <InputField
              ...
              onChange={(e) => setHouseholdTitle(e.target.value)}
            />
            ...
            <InputField
              ...
              onChange={(e) => setHouseholdBrand(e.target.value)}
            />
          
            <PostDropdown
              label="Style"
              value={householdStyle}
              onChange={setHouseholdStyle}
              options={[
                "Modern",
                "Classic",
                "Minimalist",
            ...
            />
             ...
             <InputField 
              ...
              onChange={(e) => setHouseholdColor(e.target.value)}
