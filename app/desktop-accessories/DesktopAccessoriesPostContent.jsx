...
export default function DesktopAccessoriesPostContent() {
  ...
  const [laptopConnectivityType, setLaptopConnectivityType] = useState([]);
  ...
  useEffect(() => {
    ...
    if (!isEditMode && isDraftMode && !idToUse) {
     ...
     let carAd = null;
       ...
       const connect = Array.isArray(laptopAd.laptopConnectivityType)
            ? laptopAd.laptopConnectivityType
            : [];
      setLaptopConnectivityType(connect);
     ...
     const businessId = laptopAd.businessCategory?._id 
       ...
       setEditingCarAd({
         ...
         category: carAd?.category || 'Computer Accessories',
  ...
  const buildPayload = (planType, useWallet = false) => {
    const payload = {
      ...
      laptopBrand,
      capacity,
      laptopConnectivityType: Array.isArray(laptopConnectivityType)
        ? laptopConnectivityType.map((f) => (typeof f == "string" ? f : f.name))
        : [],
      laptopColor,
      laptopWarranty,
  ...
  const submitAd = useCallback(async (planToSubmit, useWallet = false) => {
    ...
    laptopTitle, laptopType, condition, laptopBrand, capacity, laptopConnectivityType, laptopColor, laptopWarranty, amount, negotiation, business, description,
    ...
    <>
      <div className="bg-white shadow-phenom rounded-[12px] p-5 md:p-10">
        ...
        <h3 className="text-[#525252] font-[500] font-inter text-[16px] mb-4 text-left md:text-center">
          {editingCarAd ? "Complete Your Computer Accessories Ad" : "Post Computer Accessories Ad"}
        </h3>
        ...
        <form>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
            ...
            <MultiSelectDropdown
              ...
              value={laptopConnectivityType}
              onChange={setLaptopConnectivityType}
              options={[
                "Wired",
                "Wireless",
                "Bluetooth",
