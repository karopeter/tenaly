...
export default function BeautySkincarePostContent() {
  ...
  useEffect(() => {
    ...
    if (!isEditMode && isDraftMode && !idToUse) {
     ...
     let carAd = null;
       ...
       setSkinType(beautyAd.skinType || "");
       setTargetConcern(beautyAd.targetConcern || "");
     ...
     const businessId = beautyAd.businessCategory?._id 
       ...
       setEditingCarAd({
         ...
         category: carAd?.category || 'Skin Care',
  ...
  const submitAd = useCallback(async (planToSubmit, useWallet = false) => {
   ...
   beautyTitle, beautyType, condition, gender, skinType, targetConcern, beautyBrand, amount, negotiation, business, description,
    ...
    <>
      <div className="bg-white shadow-phenom rounded-[12px] p-5 md:p-10">
        ...
        <h3 className="text-[#525252] font-[500] font-inter text-[16px] mb-4 text-left md:text-center">
          {editingCarAd ? "Complete Your Skin care Ad" : "Post Skin Care Ad"}
        </h3>
