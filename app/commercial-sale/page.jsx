...
export default function MorePropertyPost() {
  ...
  // Modified useEffect to only run client-side specific logic when mounted
  useEffect(() => {
    if (!mounted) { // Only run this effect if the component is mounted on the client
      return;
    }
  ...
  useEffect(() => {
    ...
    revalidateProfile();
  }, [token, login, mounted]); // Added mounted to dependencies
  ...
  }, [propertyName, propertyAddress, propertyType, furnishing, propertyCondition, parking, squareMeter, ownershipStatus, serviceCharge, serviceFee, amount, negotiation, business, description, token, login, router]); // Added router to dependencies
  ...
  return (
    <>
    <div className="px-4 md:px-[104px] md:ml-10 mt-10 md:mt-40">
      ...
      <div className="bg-white shadow-phenom rounded-[12px] p-4 sm:p-6 md:p-10">
        ...
        <form>
          ...
          <div className="flex justify-center mt-6">
            <Button
              type="button"
              onClick={handlePost}
              className="w-full md:w-[262px] h-[44px] rounded-[8px] font-[500] text-[14px] bg-gradient-to-r from-[#00A8DF] to-[#1031AA] text-white"
            >
              Post Ad
            </Button>
          </div>
        ...
        <div className="text-center mt-6 font-[400] font-inter text-[12px] px-2">
          <p className="text-[#767676]">
            By clicking on Post Ad, you accept to
          ...
          <>
            ...
            {showFreeCommercialPropertySuccessModal && (
              <FreePropertySuccessModal
                onClose={() => setShowFreeCommercialPropertyModal(false)} // Corrected state setter
              />
            )}
          </>
        )}
