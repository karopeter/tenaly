...
export default function BeautyGroomingPostContent() {
  ...
  const [beautyPowerSource, setBeautyPowerSource] = useState("");
  ...
  useEffect(() => {
    ...
    if (!isEditMode && isDraftMode && !idToUse) {
      ...
      try {
        const beautyResponse = await api.get(`/beauty/draft/${idToUse}`);
                 
       if (!beautyResponse.data || !beautyResponse.data.beautyAd) {
         console.log("⚠️ No Beauty draft found");
         setIsLoadingDraft(false);
         return;
      }   
         
      const beautyAd = beautyResponse.data.beautyAd;
      console.log("✅ Loaded BeautyAd draft:", beautyAd);
     ...
     let carAd = null;
       ...
       setBeautyTitle(beautyAd.beautyTitle || "");
       setBeautyType(beautyAd.beautyType || "");
       setCondition(beautyAd.condition || "");
       setGender(beautyAd.gender || "");
       setBeautyPowerSource(beautyAd.beautyPowerSource || "");
       setBeautyBrand(beautyAd.beautyBrand || "");
      setAmount(beautyAd.amount?.toString() || "");
      setNegotiation(beautyAd.negotiation || "");
     setDescription(beautyAd.description || "");
         
     const businessId = beautyAd.businessCategory?._id 
           || beautyAd.businessCategory 
       ...
       setEditingCarAd({
         ...
         category: carAd?.category || 'Personal Grooming Devices',
  ...
  const submitAd = useCallback(async (planToSubmit, useWallet = false) => {
    try {
      ...
      const res = await api.post("/beauty/create-beauty-ad", payload);
   ...
   beautyTitle, beautyType, condition, beautyPowerSource, gender, beautyBrand, amount, negotiation, business, description,
  ...
  const handleSaveAsDraft = useCallback(async () => {
    try {
      ...
      const res = await api.post("/beauty/save-draft", payload);
    ...
    <>
      <div className="bg-white shadow-phenom rounded-[12px] p-5 md:p-10">
        ...
        <h3 className="text-[#525252] font-[500] font-inter text-[16px] mb-4 text-left md:text-center">
          {editingCarAd ? "Complete Your Personal Grooming Ad" : "Post Personal Grooming Ad"}
        </h3>
        ...
        <form>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
            ...
            />
             ...
             <PostDropdown 
             label="Power Source"
             value={beautyPowerSource}
             onChange={setBeautyPowerSource}
             options={[
               "Eletric",
               "Rechargeable",
               "Battery"
            ...
            <InputField
              label="Brand"
              value={beautyBrand}
              onChange={(e) => setBeautyBrand(e.target.value)}
            />
          </div>
