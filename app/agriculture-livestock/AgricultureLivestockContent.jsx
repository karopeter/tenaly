...
export default function AgricultureLivestockContent() {
        ...
        // ✅ Also fetch CarAd for images and location
         let carAd = null;
           try {
             ...
             setAgricultureType(agriAd.agricultureType?.[0] || "");
             setCondition(agriAd.condition || "");
             setUnit(agriAd.unit || "");
             if (agriAd.bulkPrice && agriAd.bulkPrice.length > 0) {
               setBulkPrices(agriAd.bulkPrice.map(bp => ({
             ...
             setEditingCarAd({
               ...
               category: carAd?.category || 'Livestock (poultry, goats, cattle, pigs, etc.)',
...
const handleRemoveBulkPrice = (index) => {
  ...
  }, [profile, submitAd, title, agricultureType, unit, condition, amount]);
  ...
  const handleSaveAsDraft = useCallback(async () => {
    try {
      ...
      //localStorage.setItem('editingAdType', 'livestock');
      ...
      toast.success(`Pets ad saved as draft with ${savedPlan} plan!`);
    ...
    <>
      <div className="bg-white shadow-phenom rounded-[12px] p-5 md:p-10">
        ...
        <h3 className="text-[#525252] font-[500] font-inter text-[16px] mb-4 text-left md:text-center">
          {editingCarAd ? "Complete Your Agriculture Ad" : "Post Livestock Ad"}
        </h3>
        ...
        <form>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
            <InputField
              label="Title"
              placeholder="E.g Healthy Broiler Chickens"
              value={title}
            ...
            <PostDropdown
              ...
              options={[
                "Poultry(Chickens, Turkeys, Ducks, Guinea Fowl)",
                "Goats",
                "Sheep",
            ...
            />
              ...
              <button
                ...
                className="flex items-center gap-2 md:mt-8 text-[#000087] font-[500] text-[14px]"
              >
               <Img
                src="/add-bulk.svg"
                alt="Add Bulk"
              ...
              {bulkPrices.length > 0 && (
                <div className="mt-3 space-y-2">
                  {bulkPrices.map((bp, index) => (
                    <div key={index} className="flex items-center justify-between bg-[#EDEDED] p-3 rounded-lg">
                      <span className="text-[14px] text-[#525252] font-[600]">
                        {bp.quantity} {bp.unit} - ₦{parseFloat(bp.amountPerUnit).toLocaleString()} per unit
                      </span>
                      <button
                        type="button"
                        onClick={() => handleRemoveBulkPrice(index)}
                        className="text-[#CB0D0D] hover:text-red-700 text-sm"
                      >
                      <Img 
                        src="/trash.svg"
                        alt="Trash Image"
  ...
  <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div className="bg-white rounded-lg p-6 w-[90%] max-w-[500px]">
      <h3 className="text-[18px] font-[600] text-[#525252] mb-4">Bulk Price</h3>
      
      <div className="space-y-4">
        <div>
          <label className="block mb-1 text-[#525252] font-[500] text-[14px]">Quantity</label>
          <input
            type="text"
            placeholder="From | Eg. 5-12"
            value={currentBulkPrice.quantity}
            onChange={(e) => setCurrentBulkPrice({...currentBulkPrice, quantity: e.target.value})}
            className="w-full border border-[#CDCDD7] rounded-[4px] px-3 py-2 focus:outline-none"
          />
        </div>

        <div>
          <label className="block mb-1 text-[#525252] font-[500] text-[14px]">Unit</label>
          <select
            value={currentBulkPrice.unit}
            onChange={(e) => setCurrentBulkPrice({...currentBulkPrice, unit: e.target.value})}
            className="w-full border border-[#CDCDD7] rounded-[4px] px-3 py-2 focus:outline-none bg-white"
          >
        ...
        <div>
          <label className="block mb-1 text-[#525252] font-[500] text-[14px]">Amount per unit</label>
          <input
            type="text"
            placeholder="₦ Enter your amount"
            value={currentBulkPrice.amountPerUnit}
            onChange={(e) => {
              const value = e.target.value;
              if (/^\d*\.?\d*$/.test(value)) {
                setCurrentBulkPrice({...currentBulkPrice, amountPerUnit: value});
              }
            }}
            className="w-full border border-[#CDCDD7] rounded-[4px] px-3 py-2 focus:outline-none"
          />
      ...
      <div className="flex gap-3 mt-6">
        <button
          ...
          className="flex-1 border border-[#CDCDD7] text-[#525252] py-2 rounded-lg font-[500]"
        >
          Cancel
        </button>
        <button
          ...
          className="flex-1 bg-gradient-to-r from-[#00A8DF] to-[#1031AA] text-white py-2 rounded-lg font-[500]"
        >
          Add
        </button>
      </div>
