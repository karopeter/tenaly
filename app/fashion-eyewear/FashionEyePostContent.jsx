...
export default function FashionEyePostContent() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const { profile, token, login } = useAuth();

  // Form states
 const [fashionTitle, setFashionTitle] = useState("");
 ...
 const [fashionColor, setFashionColor] = useState("");
  ...
  useEffect(() => {
    ...
    if (!isEditMode && isDraftMode && !idToUse) {
      ...
      try {
       const fashionResponse = await api.get(`/fashion/draft/${idToUse}`);
                 
       if (!fashionResponse.data || !fashionResponse.data.fashionAd) {
         console.log("⚠️ No Fashion draft found");
         setIsLoadingDraft(false);
         return;
      }   
         
      const fashionAd = fashionResponse.data.fashionAd;
      console.log("✅ Loaded FashionAd draft:", fashionAd);
     ...
     let carAd = null;
       ...
       setFashionTitle(fashionAd.fashionTitle || "");
       setFashionType(fashionAd.fashionType || "");
       setCondition(fashionAd.condition || "");
       setFashionBrand(fashionAd.fashionBrand || "");
       setGender(fashionAd.gender || "");
       setFrameShape(fashionAd.frameShape || "");
       setFrameMaterial(fashionAd.frameMaterial || "");
       setFashionAccessories(fashionAd.fashionAccessories || "");
       setLensType(fashionAd.lensType || "");
       setFashionColor(fashionAd.fashionColor || "");
      setAmount(fashionAd.amount?.toString() || "");
      setNegotiation(fashionAd.negotiation || "");
     setDescription(fashionAd.description || "");
         
     const businessId = fashionAd.businessCategory?._id 
           || fashionAd.businessCategory 
       ...
       setEditingCarAd({
         ...
         category: carAd?.category || 'Eyewear (Glasses & Sunglasses)',
         location: carAd?.location || '',
         images: carAd?.fashionImage || [],
      });
  ...
  const submitAd = useCallback(async (planToSubmit, useWallet = false) => {
    try {
      ...
      const res = await api.post("/fashion/create-fashion-ad", payload);
    ...
    fashionTitle, fashionType, condition, fashionBrand, gender, frameShape, frameMaterial, fashionAccessories, lensType, fashionColor,  amount, negotiation, business, description,
  ...
  const handleSaveAsDraft = useCallback(async () => {
    try {
      ...
      const res = await api.post("/fashion/save-draft", payload);

      const savedPlan = res.data.data?.plan || 'free';

      toast.success(`Fashion ad saved as draft with ${savedPlan} plan!`);
    ...
    <>
      <div className="bg-white shadow-phenom rounded-[12px] p-5 md:p-10">
        ...
        <h3 className="text-[#525252] font-[500] font-inter text-[16px] mb-4 text-left md:text-center">
          {editingCarAd ? "Complete Your Eyewear Ad" : "Post Eyewear Ad"}
        </h3>
        ...
        <form>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
           ...
           <InputField 
            label="Accessories Included (Optional)"
            value={fashionAccessories}
            onChange={(e) => setFashionAccessories(e.target.value)}
            placeholder="E.g Case, Cleaning Cloth, Lens Wipes, Others"
           />
