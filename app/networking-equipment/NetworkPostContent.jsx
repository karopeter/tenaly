...
export default function NetworkPostContent() {
  ...
  useEffect(() => {
    ...
    if (!isEditMode && isDraftMode && !idToUse) {
     ...
     let carAd = null;
       ...
       setSpeedRating(laptopAd.speedRating || "");
     ...
     const businessId = laptopAd.businessCategory?._id 
       ...
       setEditingCarAd({
         ...
         category: carAd?.category || 'Networking Equipment',
  ...
  const submitAd = useCallback(async (planToSubmit, useWallet = false) => {
    ...
    laptopTitle, laptopType,  condition, laptopBrand, speedRating,  laptopWarranty, amount, negotiation, business, description,
    ...
    <>
      <div className="bg-white shadow-phenom rounded-[12px] p-5 md:p-10">
        ...
        <h3 className="text-[#525252] font-[500] font-inter text-[16px] mb-4 text-left md:text-center">
          {editingCarAd ? "Complete Your Networking Equipment Ad" : "Post Networking Equipment Ad"}
        </h3>
