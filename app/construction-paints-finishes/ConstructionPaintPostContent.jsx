...
export default function ConstructionPaintPostContent() {
  ...
  const [constructionColor, setConstructionColor] = useState("");
  ...
  useEffect(() => {
    ...
    if (!isEditMode && isDraftMode && !idToUse) {
     ...
     let carAd = null;
       ...
       setFinish(constructionAd.finish || "");
       setConstructionColor(constructionAd.constructionColor || "");
     ...
     const businessId = constructionAd.businessCategory?._id 
       ...
       setEditingCarAd({
         ...
         category: carAd?.category || 'Paints & Finishes',
  ...
  const submitAd = useCallback(async (planToSubmit, useWallet = false) => {
   ...
   constructionTitle, constructionType, constructionMaterial, condition, finish, constructionColor, size,  constructionBrand,  amount, negotiation, business, description,
    ...
    <>
      <div className="bg-white shadow-phenom rounded-[12px] p-5 md:p-10">
        ...
        <h3 className="text-[#525252] font-[500] font-inter text-[16px] mb-4 text-left md:text-center">
          {editingCarAd ? "Complete Your Paints & Finishes Ad" : "Post Paints & Finishes Ad"}
        </h3>
        ...
        <form>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
            ...
            label="Color"
            placeholder="Color"
            value={constructionColor}
            onChange={(e) => setConstructionColor(e.target.value)}
            />
          </div>
