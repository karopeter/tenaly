...
export default function HouseholdLightPostContent() {
 ...
 const [householdBrand, setHouseholdBrand] = useState("");
  ...
  useEffect(() => {
    ...
    if (!isEditMode && isDraftMode && !idToUse) {
     ...
     let carAd = null;
       ...
       setPowerType(householdAd.powerType || "");
       setColorTemperature(householdAd.colorTemperature || "");
     ...
     const businessId = householdAd.businessCategory?._id 
       ...
       setEditingCarAd({
         ...
         category: carAd?.category || 'Lighting',
  ...
  const submitAd = useCallback(async (planToSubmit, useWallet = false) => {
    ...
    householdTitle, householdType, condition, householdBrand, powerType, colorTemperature, amount, negotiation, business, description,
    ...
    <>
      <div className="bg-white shadow-phenom rounded-[12px] p-5 md:p-10">
        ...
        <h3 className="text-[#525252] font-[500] font-inter text-[16px] mb-4 text-left md:text-center">
          {editingCarAd ? "Complete Your Lighting Ad" : "Post Lighting Ad"}
        </h3>
        ...
        <form>
         ...
         <PostDropdown 
            ...
            <InputField
              ...
              onChange={(e) => setHouseholdBrand(e.target)}
              placeholder="Eneter"
            />


            <PostDropdown 
              label="Coloro Temperature"
