...
export default function ConstructionRepairPostContent() {
  ...
  const [constructionAvailability, setConstructionAvailability] = useState("");
  ...
  useEffect(() => {
    ...
    if (!isEditMode && isDraftMode && !idToUse) {
     ...
     let carAd = null;
       ...
       setExperienceLevel(constructionAd.experienceLevel || "");
       setConstructionAvailability(constructionAd.constructionAvailability || "");
     ...
     const businessId = constructionAd.businessCategory?._id 
       ...
       setEditingCarAd({
         ...
         category: carAd?.category || 'Repair & Maintenance Services',
  ...
  const submitAd = useCallback(async (planToSubmit, useWallet = false) => {
   ...
   constructionTitle, constructionType, experienceLevel, constructionAvailability, amount, negotiation, business, description,
    ...
    <>
      <div className="bg-white shadow-phenom rounded-[12px] p-5 md:p-10">
        ...
        <h3 className="text-[#525252] font-[500] font-inter text-[16px] mb-4 text-left md:text-center">
          {editingCarAd ? "Complete Your Repair & Maintenance  Ad" : "Post Repair & Maintenance Service Ad"}
        </h3>
        ...
        <form>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
           ...
           <PostDropdown 
             label="Availiability"
             value={constructionAvailability}
             onChange={setConstructionAvailability}
             options={[
              "Full-Time",
              "Part-Time",
              "On Appointment",
