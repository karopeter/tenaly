...
export default function KitchenResturantPostContent() {
 ...
 const [amount, setAmount] = useState(""); 
  ...
  useEffect(() => {
    const fetchDraftData = async () => {
      ...
      try  {
        ...
        const equipmentResponse = await api.get(`/equipments/draft/${idToUse}`);
        
        if (!equipmentResponse.data || !equipmentResponse.data.equipmentAd) {
          console.log("⚠️ No EquipmentAd draft found");
          setIsLoadingDraft(false);
          return;
        }

        const equipmentAd = equipmentResponse.data.equipmentAd;
        console.log("✅ Loaded KidsAd draft:", equipmentAd);
       ...
       setDescription(equipmentAd.description || "");
        ...
        setEditingCarAd({
          ...
          category: carAd?.category || 'Kitchen & Restaurant Equipment',
          location: carAd?.location || '',
          images: carAd?.equipmentImage || [],
        });
  ...
  const submitAd = useCallback(async (planToSubmit, useWallet = false) => {
    try {
      ...
      const res = await api.post("/equipments/create-equipment-ad", payload);
  ...
  const handleSaveAsDraft = useCallback(async () => {
    try {
      ...
      const res = await api.post("/equipments/save-draft", payload);

      const savedPlan = res.data.data?.plan || 'free';

      toast.success(`Equipment ad saved as draft with ${savedPlan} plan!`);
    ...
    <>
      <div className="bg-white shadow-phenom rounded-[12px] p-5 md:p-10">
        ...
        <h3 className="text-[#525252] font-[500] font-inter text-[16px] mb-4 text-left md:text-center">
          {editingCarAd ? "Complete Ads for Kitchen Equipment" : "Post Kitchen Eqiupment tools Ad"}
        </h3>
