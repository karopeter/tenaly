...
export default function DesktopPostContent() {
  ...
  useEffect(() => {
    ...
    if (!isEditMode && isDraftMode && !idToUse) {
     ...
     let carAd = null;
       ...
       setResolution(laptopAd.resolution || "");
       setRefreshRate(laptopAd.refreshRate || "");
     ...
     const businessId = laptopAd.businessCategory?._id 
       ...
       setEditingCarAd({
         ...
         category: carAd?.category || 'Monitors',
  ...
  const submitAd = useCallback(async (planToSubmit, useWallet = false) => {
    ...
    laptopTitle,  condition, laptopBrand,  laptopScreenSize, resolution, refreshRate, laptopColor,  laptopWarranty, amount, negotiation, business, description,
    ...
    <>
      <div className="bg-white shadow-phenom rounded-[12px] p-5 md:p-10">
        ...
        <h3 className="text-[#525252] font-[500] font-inter text-[16px] mb-4 text-left md:text-center">
          {editingCarAd ? "Complete Your  Monitor Ad" : "Post Monitor Ad"}
        </h3>
              ...
              >
               {isSavingDraft ? (
                 <span className="flex items-center justify-center">
                   <span className="animate-spin rounded-full h-4 w-4 border border-b-2 border-gray"></span>
                   Saving...
                 </span>
               ): "Save as Draft"}
              </Button>
