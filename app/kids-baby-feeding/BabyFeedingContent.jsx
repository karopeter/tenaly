...
export default function BabyFeedingPostContent() {
 ...
 const [amount, setAmount] = useState(""); 
    ...
    const fetchDraftData = async () => {
      ...
      try {
        ...
        setEditingCarAd({
          ...
          category: carAd?.category || 'Feeding (bottles, high chairs, breast pumps)',
  ...
  const submitAd = useCallback(async (planToSubmit, useWallet = false) => {
    try {
      ...
      const res = await api.post("/kids/create-kids-ad", payload);
  ...
  const handleSaveAsDraft = useCallback(async () => {
    try {
      ...
      const res = await api.post("/kids/save-draft", payload);

      const savedPlan = res.data.data?.plan || 'free';

      toast.success(`Kids ad saved as draft with ${savedPlan} plan!`);
    ...
    <>
      <div className="bg-white shadow-phenom rounded-[12px] p-5 md:p-10">
        ...
        <h3 className="text-[#525252] font-[500] font-inter text-[16px] mb-4 text-left md:text-center">
          {editingCarAd ? "Complete For Kids Ad" : "Post For Kids Ad"}
        </h3>
        ...
        <form>
          ...
          <div className="mt-4">
            ...
            <textarea
              placeholder="About details about quality, size/measurements, and any defects"
              value={description}
